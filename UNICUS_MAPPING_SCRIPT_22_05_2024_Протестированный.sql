...
INSERT INTO IDM_MGMT_MAP.UNC_UU_MAP_ETALON (
    ...
    WITH FOR_REFERENCE AS (
    	SELECT
    		...
    		ENTRY_ID,
    		POLICY_ID_100
    	FROM STG_UNC.XX_ALFA_RZ_CONTRACT
    ...
    , PRODUCT_ATTRIBUTES AS (
       ...
       , UTK AS (
            ...
            LEFT JOIN IDM_MNGL_MAPPING."UNICUS Для Эталона" FP
				...
				CONCAT(FP.POLICY_UNIQUE_ID_036, '@', FP.ENTRY_ID) = CONCAT(LP.POLICY_UNIQUE_ID_036, '@', LP.ENTRY_ID) AND
				"Код OEBS АВС 2011" IS NULL
...
CREATE OR REPLACE TABLE IDM_MNGL_MAPPING."UNICUS Доп Мэппинг 2011" AS (
