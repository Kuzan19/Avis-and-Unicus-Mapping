...
CREATE OR REPLACE TABLE IDM_MNGL_MAPPING."Журнал Авис" AS (
    ...
    SELECT
    	...
    	ABC."Номер строчки",
    	TO_CHAR(ABC."Код OEBS АВС 2011") "Код АВС",
    	KP."Код OEBS КП 2011",
    	AGR.GROUPE_ID,
    	AGRN.GROUPE_NAME,
    	CASE
	    	WHEN AGRN.GROUPE_NAME LIKE '%Сбербанк%' THEN 'B301007'
	    	WHEN (TMP.AG_OUR_NUMBER LIKE '%/459/%' OR TMP.AG_OUR_NUMBER LIKE '%/460/%')
	    	    AND TMP.AG_OUR_NUMBER NOT LIKE '%/045/459/%'
	    	    AND TMP.AG_OUR_NUMBER NOT LIKE '%/045/460/%'
	    	THEN 'B301001'
	    	ELSE 'B301000'
	    END "Продукт OEBS"
    FROM (SELECT
        ...
        FROM IDM_MNGL_MAPPING.AVIS
        WHERE (AC_DEBIT = '22/5Н' OR AC_DEBIT = '91/17') AND
         (IS_LOADED <> 'не начисл. в Парус' OR IS_LOADED IS NULL)) TMP
    ...
    LEFT JOIN STG_AVIS.AGREEMENT AGR
        ON AGR.AG_ID = TMP."Договор ID"

    LEFT JOIN STG_AVIS.AGREEMENT_GROUP_NEW AGRN
        ON AGRN.GROUPE_ID = TMP."Группа"
);
...
INSERT INTO IDM_MNGL_MAPPING.AVIS_MAPP(
    ...
    SELECT
        ...
        '2' "Год договора OEBS"
    FROM IDM_MNGL_MAPPING."Журнал Авис" AMG
    GROUP BY "Корневой договор номер "
    WHERE NOT EXISTS(
        SELECT
            1
        FROM IDM_MNGL_MAPPING.AVIS_MAPP AM
        WHERE AM."ag_our_number" = AMG."Корневой договор номер "
);
