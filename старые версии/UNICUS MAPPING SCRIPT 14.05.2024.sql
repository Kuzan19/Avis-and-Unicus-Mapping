...
--		ON SUB.SUBJECT_ID = CON.MANAGER_SUBJECT_ID
    ...
    WHERE ARC.PREMIUM_CHARGE_DATE_051 BETWEEN '2024-01-01 01:00:00.0' AND '2024-01-05 01:00:00.0'
);

CREATE OR REPLACE TABLE IDM_MNGL_MAPPING."UNICUS Не Эталон" AS (
    SELECT
        *,
        '1' "Эталон"
	...
	WITH LAST_POLIS AS (
		...
		FROM (
		    ...
		    WHERE NOT EXISTS (
		        ...
		        WHERE T2.POLICY_UNIQUE_ID_036 = POLICY_ID_100 AND (T1.POLICY_UNIQUE_ID_036 = T2.POLICY_UNIQUE_ID_036)
		        )
		    GROUP BY POLICY_UNIQUE_ID_036
	        )
