...
--Создаем временную таблицу для таблицы "Журнал Авис"
CREATE OR REPLACE TABLE IDM_MNGL_MAPPING.AVIS_JOURNAL AS (
    ...
    SELECT
	    ...
	    TO_CHAR(STG_FILES."015_справ_кодов_КП"."Код OEBS КП 2011") "КП",
	    GROUPE_ID,
	    GROUPE_NAME
    FROM (
        SELECT
            COMPANY "Филиал код",
    	    TO_CHAR(ID_DEPARTMENT) "Подразделение код",
        ...
        SELECT
        	...
        	TO_DATE(ECON_DATE) "Дата подтверждения",
        	OPR_SUM_R "Сумма",
        	OPR_SUM_D "Сумма_депозит",
        	TO_DATE(
        	    CASE
        	    	WHEN AM_START_DATE IS NULL THEN OP_DATE
        	    	ELSE AM_START_DATE
        	    END
	...
	LEFT JOIN STG_FILES."017_справ_кодов_ЦФО" CFO
		ON TO_CHAR(TMP."Подразделение код") = TO_CHAR(CFO."код_подразделения") AND CFO."код_подразделения" IS NOT NULL
    ...
    SELECT IDM_MNGL_MAPPING.AVIS_JOURNAL.*,
	    CASE
	    	...
	    	WHEN ("номер корневого договора" LIKE '%/459/%' OR "номер корневого договора" LIKE '%/460/%')
	    	    ...
	    	    OR "номер корневого договора" NOT LIKE '%/045/460/%')
	    	THEN 'B301001'
	    	ELSE 'B301000'
	    END "Продукт OEBS"
    FROM IDM_MNGL_MAPPING.AVIS_JOURNAL
);
...
INSERT INTO IDM_MNGL_MAPPING.AVIS_MAPP(
    ...
    WITH AVIS_MAPP_GR AS (
        SELECT
        	...
        	FIRST_VALUE("Код АВС") "КОД OEBS АВС 2011",
        	FIRST_VALUE("Код ЦФО") "КОД OEBS ЦФО 2011",
        	FIRST_VALUE("Код OEBS КП 2011") "КП 2011",
        	FIRST_VALUE("Подразделение код") "КОД ПП GR",
        	FIRST_VALUE("Продукт OEBS") "Продукт OEBS"
